...
function angleBetweenClockHands(date) {
  const arrowHour = ((date.getUTCHours() % 12) * 60) + date.getMinutes();
  const arrowMinute = date.getMinutes() * 12;
  const angle = Math.abs(arrowHour - arrowMinute);
  return (angle > 360 ? angle - 360 : angle) * (Math.PI / 360);
}
